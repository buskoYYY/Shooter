...
namespace KINEMATION.RetargetPro.Editor.Scripts.Menus
{
    public class ProfileContextMenu
    {
...
#endif
            rig.rigElementChains = GenerateChainsFromSkeletonBones(skeletonBones);
        ...
        private static List<KRigElementChain> GenerateChainsFromSkeletonBones(List<KRigElement> skeletonBones)
        {
            ...
            if (RetargetPresetUtility.TryMatchPreset(tempRig, out RetargetPreset preset))
            {
        ...
        {
            if (EditorUtility.InstanceIDToObject(instanceId) is not RetargetProfile profile)
            {
                return false;
            }
